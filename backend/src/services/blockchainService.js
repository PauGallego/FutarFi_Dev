...
class BlockchainService {
  ...
  async initialize() {
    try {
      ...
      console.log('📋 ProposalManager loaded:', proposalManagerAddress);
  ...
  setupEventListeners() {
    ...
    this.proposalManagerContract.on('ProposalCreated', async (proposalId, admin, proposalAddress, event) => {
      console.log(`🆕 New proposal detected: ID=${proposalId}, Admin=${admin}, Address=${proposalAddress}`);
      
      try {
        await this.saveProposal(proposalAddress);
        console.log(`✅ New proposal saved: ${proposalAddress}`);
      } catch (error) {
        console.error(`❌ Error saving new proposal ${proposalAddress}:`, error);
      }
    });
  }

  async importExistingProposals() {
    console.log('📥 Importing existing proposals...');
    
    try {
      // Get all proposal addresses from blockchain
      const proposalAddresses = await this.proposalManagerContract.getAllProposals();
      console.log(`📋 Found ${proposalAddresses.length} proposals on blockchain`);
      
      if (proposalAddresses.length === 0) {
        console.log('📋 No proposals to import');
        return;
      }
      ...
      for (let i = 0; i < proposalAddresses.length; i++) {
        ...
        console.log(`📋 Processing proposal ${i + 1}/${proposalAddresses.length}: ${address}`);
        
        try {
          await this.saveProposal(address);
          console.log(`✅ Imported proposal: ${address}`);
        } catch (error) {
          console.error(`❌ Error importing proposal ${address}:`, error);
        }
      }
      
      console.log('✅ Finished importing existing proposals');
    } catch (error) {
      console.error('❌ Error importing existing proposals:', error);
    }
  }
  ...
  async checkForNewProposals() {
    ...
    try {
      // Get all proposal addresses from blockchain
      const blockchainProposals = await this.proposalManagerContract.getAllProposals();
      
      // Get proposals from database
      const dbProposals = await Proposal.find({}).select('contractAddress');
      const dbAddresses = dbProposals.map(p => p.contractAddress.toLowerCase());
      
      // Find new proposals
      const newProposals = blockchainProposals.filter(
        address => !dbAddresses.includes(address.toLowerCase())
      );
      
      if (newProposals.length > 0) {
        console.log(`🆕 Found ${newProposals.length} new proposals to import`);
        
        for (const address of newProposals) {
          try {
            await this.saveProposal(address);
            console.log(`✅ New proposal imported: ${address}`);
          } catch (error) {
            console.error(`❌ Error importing new proposal ${address}:`, error);
          }
        }
      }
  ...
  async saveProposal(proposalAddress) {
    try {
      ...
      if (existingProposal) {
        console.log(`📋 Proposal ${proposalAddress} already exists in database`);
      ...
      console.log(`💾 Saved proposal to database: ${proposalData.name}`);
      return proposal;
      
    } catch (error) {
      console.error(`❌ Error saving proposal ${proposalAddress}:`, error);
      throw error;
    }
  }
  ...
  async getProposalData(proposalAddress) {
    try {
      console.log(`🔍 Getting details for proposal: ${proposalAddress}`);
      ...
      console.log(`📋 Proposal basic data: ${name} (ID: ${id}), Active: ${isActive}`);
      ...
      console.log(`💰 Initial prices - Approve: ${approvePrice}, Reject: ${rejectPrice}`);
      ...
      console.error(`❌ Error getting proposal data for ${proposalAddress}:`, error);
      throw error;
    }
  }
  ...
  async updateProposalPrices() {
    ...
    try {
      ...
      console.log(`📊 Updating prices for ${proposals.length} proposals...`);
      
      for (const proposal of proposals) {
        try {
          await this.updateSingleProposalPrice(proposal);
        } catch (error) {
          console.error(`❌ Error updating prices for proposal ${proposal.contractAddress}:`, error);
        }
      }
  ...
  async updateSingleProposalPrice(proposal) {
    try {
      ...
      if (!isActive && proposal.status === 'active') {
        ...
        await proposal.save();
        console.log(`📋 Proposal ${proposal.name} marked as ended`);
      ...
      if (!isMarketOpen) {
        console.log(`📋 Market for proposal ${proposal.name} is closed`);
        return;
      }
      ...
      if (pricesChanged) {
        console.log(`💰 Price change detected for ${proposal.name}:`);
        console.log(`   Approve: ${proposal.currentApprovePrice} → ${newApprovePriceStr}`);
        console.log(`   Reject: ${proposal.currentRejectPrice} → ${newRejectPriceStr}`);
        ...
        console.log(`💾 Updated prices for proposal: ${proposal.name}`);
      }
      
    } catch (error) {
      console.error(`❌ Error updating price for proposal ${proposal.contractAddress}:`, error);
    }
  }

  // Start monitoring every 5 seconds for both new proposals and price updates
  startMonitoring() {
    console.log('🔄 Starting monitoring (every 5 seconds)...');
    console.log('   - Checking for new proposals');
    console.log('   - Updating prices for existing proposals');
    ...
    setInterval(async () => {
      try {
        ...
        // Then update prices for all proposals
  ...
  getMarketABI() {
    return [
      ...
      "function buy(uint8 idx, uint256 amount) external"
    ];
  }
}
